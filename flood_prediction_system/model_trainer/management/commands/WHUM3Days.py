...
class Command(BaseCommand):
    help = 'Forecast humidity for the next three days using LSTMV1 model'

    def handle(self, *args, **kwargs):
        # Fetch and preprocess humidity data
        self.stdout.write("Fetching humidity data...")
        weather_data = WeatherData.objects.all().values('datetime', 'humidity')
        ...
        scaler.adapt(np.array(weather_df[['humidity']]))
        normalized_data = scaler(np.array(weather_df[['humidity']]))
        ...
        # Print the last 10 days of humidity data
        self.stdout.write("\nLast 10 Days of Humidity:")
        last_10_days = weather_df.tail(10)
        for date, humidity in zip(last_10_days.index, last_10_days['humidity']):
            self.stdout.write(f"{date.strftime('%Y-%m-%d')}: {humidity:.2f}%")

        # Print the 3-day forecast
        self.stdout.write("\n3-Day Humidity Forecast:")
        ...
        for date, forecast in zip(forecast_dates, predictions_unscaled.flatten()):
            self.stdout.write(f"{date.strftime('%Y-%m-%d')}: {forecast:.2f}%")
    ...
    def plot_forecast(self, weather_df, forecast):
        ...
        plt.plot(last_days.index, last_days['humidity'], label="Historical Humidity", color='blue')
        ...
        plt.plot(forecast_dates, forecast, label="3-Day Humidity Forecast", color='red', marker='o')

        # Chart formatting
        plt.xlabel("Date")
        plt.ylabel("Humidity (%)")
        plt.title("Last 10 Days of Humidity and 3-Day Forecast")
