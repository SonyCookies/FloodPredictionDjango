...
class Command(BaseCommand):
    help = 'Forecast dew point for the next three days using LSTMV1 model'

    def handle(self, *args, **kwargs):
        # Fetch and preprocess dew data
        self.stdout.write("Fetching dew data...")
        weather_data = WeatherData.objects.all().values('datetime', 'dew')
        ...
        scaler.adapt(np.array(weather_df[['dew']]))
        normalized_data = scaler(np.array(weather_df[['dew']]))
        ...
        # Print the last 10 days of dew data
        self.stdout.write("\nLast 10 Days of Dew Point:")
        last_10_days = weather_df.tail(10)
        for date, dew in zip(last_10_days.index, last_10_days['dew']):
            self.stdout.write(f"{date.strftime('%Y-%m-%d')}: {dew:.2f}°C")
    ...
    def plot_forecast(self, weather_df, forecast):
        ...
        plt.plot(last_days.index, last_days['dew'], label="Historical Dew Point", color='blue')
        ...
        plt.plot(forecast_dates, forecast, label="3-Day Dew Point Forecast", color='red', marker='o')

        # Chart formatting
        plt.xlabel("Date")
        plt.ylabel("Dew Point (°C)")
        plt.title("Last 10 Days of Dew Point and 3-Day Forecast")
